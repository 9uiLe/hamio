...
配布の失敗処理は GitHub CLI の代替実装で検証する。候補検証は実際の provenance と独立プロジェクトでの実行を確認し、公開後の導入試験は immutable release と公開資産の結び付きを確認する。試験の存在と実施済みの結果を区別し、対象ソース・環境・未確認範囲を結果に記録する。
