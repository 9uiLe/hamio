...
| ------------------------------------------------------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------------------- |
| [Clack Core](https://bomb.sh/docs/clack/packages/core/) | 入力、選択、確認、秘密入力、render 関数、カスタム入出力ストリーム、AbortSignal | フォームの端末制御に固定版1.5.1を使い、画面を hamio の renderer で生成する |
...
[JSON-RPC 2.0](https://www.jsonrpc.org/specification) は要求 ID、応答、通知、エラーを定義する。一方、メッセージの区切り、キャンセル、認証、再接続、互換版の交渉は別に設計が必要になる。hamio の API v1 はフォーム、単発表示、進捗ストリーム、機能照会を対象とし、汎用の双方向セッションを要求しない。
