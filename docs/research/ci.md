...
hamio の CI は、PR のマージ候補に対して開発基盤の品質検査を行う。通常の PR は Linux の1ジョブとし、macOS は関連変更時に手動で検証する。本書はその検証範囲と実行時間の判断根拠を記録する。操作方法は[開発手順](../development.md)、製品の性能測定は[性能特性と測定](performance.md)を参照する。
...
| 製品の端末・プロセス・配布物 | 実装時に両 OS の自動テスト範囲を定める      | 入力モード、シグナル、native 部品、ランタイムを実機で検証する |
