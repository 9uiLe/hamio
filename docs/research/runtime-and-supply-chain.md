...
hamio は UI コードを実行ファイルへ含め、利用側コードの動的読み込みと依存取得を設けない。`.env`、`bunfig.toml`、`package.json`、`tsconfig.json` の暗黙読み込みを無効化する。起動前に作用する環境変数は利用側の信頼境界に含め、`BUN_OPTIONS` と `BUN_BE_BUN` を未信頼データから設定しない。具体的なビルド設定と検証方法は[実装設計](../implementation.md)に定める。
...
これらの API は、測定用プロセスや Bun から hamio を呼ぶ連携の参考となる。hamio は UI を担当し、業務プロセスの起動・管理は利用側の責務とする。利用側は必要な出力を継続して読み、UI 終了と業務終了を別に扱う。

hamio 内では受信メッセージ、対話描画の待ち行列、活動中 task、警告の保持量を [API 契約](../api.md#資源上限と機能照会)で制限する。アプリ内の上限、プロセス全体のメモリ上限、計測結果を区別する。詳細な測定指標は [性能資料](performance.md) に記載する。
